...
For the second half of the day, you will use Docker and Docker Compose to manage two containers for the UFW and firewalld activities. 

Open a terminal and go to the specified directory.

- Type `cd ~/Cybersecurity-Lesson-Plans/11-NetSec`

   - There should be a `docker-compose.yml` inside that directory.

   - ***Note: If the directory is missing, run the following command, to download and start up the docker containers.***

`wget https://gist.githubusercontent.com/jlow3939/904eb58af3605457255df35c649f9873/raw/69bc0efdb38837ecce8db14662e9efffbfe15429/docker-compose.yml`
...
- Run `docker-compose ps` to check the statuses.
...
- Navigate to where a `docker-compose.yml` file exists. For example:
  ...
  or the directory in which you ran the `wget` command earlier in lesson.
...
Another firewall, called firewalld, is a bit more complicated than UFW, but firewalld provides greater flexibility and additional features. It's important to be familiar with both firewalls in order to be prepared for supporting them in the future. 
...
- Navigate to where a `docker-compose.yml` file exists. For example:
  ...
  - `cd ~/Cybersecurity-Lesson-Plans/11-NetSec` 
  or the directory in which you ran the `wget` command earlier.
...
In this demo, we'll use our firewalld container to perform scans against our UFW firewall.
...
1. First, we will set up our test environment. We'll establish a basic UFW configuration.
   ...
   Connect to your `ufw` container using: `docker exec -it ufw bash`
...
2. Now we'll simulate an OS scan, which will enumerate OS names and versions. With this information, attackers know which specific vulnerabilities exist then attack those.
   ...
   Type the following command from inside the firewalld container:

   Connect to the `firewalld` container using `docker exec -it firewalld bash`.

   And run the following command:

   - `sudo nmap -O -p 1-500 --osscan-guess 172.16.18.72`

      - `nmap` is the program to perform the network scan.
      - `-O` enables OS detection.
      - `-p 1-500` scans ports `1` through `500`.
      - `172.16.18.72` is the IP address to perform the network scan against.

   The output should resemble: 
   
   ```bash
   Starting Nmap 7.60 ( https://nmap.org ) at 2020-03-18 14:42 EDT
   Nmap scan report for 172.16.18.72
   ...
   From within the UFW container, type the following command:
   ...
   From within the UFW container, `enable` the UFW firewall and run the test again.

   Return to the firewalld container and try the command one more time. 
...
3. Next we'll perform an Nmap scan to enumerate specific services and daemons that are running on open ports.
   ...
   Type the following command from within the firewalld container:

   - `nmap -sV 172.16.18.72`
...
4. Next we will perform a scan that attempts to fingerprint the type of operating system running on a host by enumerating information gathered through open ports.
   ...
   Type the following command from inside the UFW container:
...
5. Attackers use different methods to determine the true state of a port, i.e., if a port is actually open or closed. Nmap has some tools for determining the true state of a port. With the UFW firewall enabled, perform the following actions:
   ...
   Type the following command with the firewalld Container:
  ...
  - This means that the scan determined that the ports are in fact accessible, but is unable to determine whether the ports are open or closed. This indicates that a stateful firewall is being used.
